...
const PROFILE_DEFINITIONS = [
  {
    ...
    tiers: ['official'],
    topics: ['struktura bazy danych', 'schemat SQL', 'zmiany wersji', 'SQL Server', 'wydajność', 'indeksy'],
    seedQuery: 'Comarch ERP Optima struktura bazy danych tabele procedury klucze obce TraNag TraElem 2026',
  },
  {
    kbNamespace: 'ComarchOptimaAdditionalFunctions',
    mode: 'CANDIDATE_ONLY',
    domains: ['partner.erp.comarch.pl', 'pomoc.comarch.pl'],
    professionalDomains: ['justlabel.io', 'erpserwis.net', 'cdn.3lance.pl'],
    tiers: ['official'],
  ...
  },
  {
    ...
    mode: 'CANDIDATE_ONLY',
    domains: ['partner.erp.comarch.pl', 'pomoc.comarch.pl'],
    professionalDomains: ['erpserwis.net', 'justlabel.io'],
    tiers: ['official'],
    topics: ['sPrint', 'wydruki', 'GenRap', 'wzorce'],
    seedQuery: 'Comarch ERP Optima sPrint konfigurator danych szablonu parametry subszablony SQL 2026',
  },
  {
    ...
    seedQuery: 'Comarch ERP Optima instrukcja obsługi konfiguracja personalizacja kolumny użytkownika 2026',
  },
  {
    kbNamespace: 'ComarchOptimaPartnerTechnical',
    mode: 'PARTNER_ROUTE',
    domains: ['partner.erp.comarch.pl'],
    professionalDomains: ['webarm.pl', 'kotrak.com', '4programmers.net', 'cdn.3lance.pl', 'erpserwis.net'],
    tiers: ['official'],
  ...
  },
  {
    ...
    professionalDomains: ['pomoc.erpxt.pl', 'oneclick-workflow.pl'],
    communityDomains: ['spolecznosc.comarch.pl'],
    tiers: ['official', 'community'],
    topics: ['API', 'KSeF', 'faktury', 'integracje', 'e-faktury', 'certyfikaty'],
    seedQuery: 'Comarch Betterfly API KSeF e-faktury integracje certyfikaty problemy 2026',
  },
  {
  ...
  },
  {
    ...
    professionalDomains: ['infor.pl', 'lexlege.pl', 'prawo.pl', 'lex.pl', 'tpa-group.pl', 'taxeo.pl', 'dudkowiak.com', 'wtssaja.pl', 'modzelewski.tax', 'inforlex.pl', 'nexiaadvicero.eu'],
    tiers: ['official', 'professional'],
    topics: ['prawo podatkowe', 'interpretacje', 'ustawy', 'objaśnienia', 'orzecznictwo'],
    seedQuery: 'prawo podatkowe interpretacje ustawy orzecznictwo zmiany 2026',
  },
  {
    kbNamespace: 'TaxbellPayrollHRReference',
    mode: 'DIRECT_DRAFT',
    domains: ['zus.pl', 'pip.gov.pl', 'gov.pl', 'biznes.gov.pl', 'praca.gov.pl'],
    professionalDomains: ['infor.pl', 'kadry.infor.pl', 'koda-advisory.pl', 'correctcontext.com', 'knowledge.dlapiper.com', 'dentons.com', 'gide.com', 'ksiegowosc365.com', 'topsourceworldwide.com', 'leglobal.law'],
    tiers: ['official', 'professional'],
    topics: ['kadry', 'płace', 'ZUS', 'prawo pracy', 'BHP', 'zatrudnienie'],
    seedQuery: 'kadry płace ZUS prawo pracy BHP zatrudnienie zmiany 2026',
  },
  {
    kbNamespace: 'TaxbellAccountingVATReference',
    mode: 'DIRECT_DRAFT',
    domains: ['podatki.gov.pl', 'ksef.podatki.gov.pl', 'gov.pl', 'biznes.gov.pl'],
    professionalDomains: ['infor.pl', 'rachunkowosc.com.pl', 'poradnikprzedsiebiorcy.pl', 'ksiegowosc.infor.pl', 'avalara.com', 'vatupdate.com', 'globalvatcompliance.com', 'taxadvisorypoland.com', 'podatnik.info', 'tpa-group.pl', 'nexiaadvicero.eu', 'wtssaja.pl', 'dudkowiak.com', 'modzelewski.tax'],
    tiers: ['official', 'professional'],
    topics: ['VAT', 'JPK', 'KSeF', 'rachunkowość', 'księgowość'],
    seedQuery: 'VAT JPK KSeF rachunkowość księgowość zmiany obowiązki 2026',
  },
];
...
export function generateQualityReport() {
  ...
  report.push(`# ERP KB Quality Weekly Report\n`);
  report.push(`**Generated:** ${now.toISOString()}\n`);
  report.push(`**Period:** ${weekAgo} — ${now.toISOString().slice(0, 10)}\n`);
  report.push(`\n---\n`);
  try {
    const alerts = detectAnomalies();
    if (alerts.length) {
      ...
      for (const alert of alerts) {
        report.push(`- [${alert.severity.toUpperCase()}] **${alert.kbNamespace}**: ${alert.message}\n`);
      }
      report.push(`\n`);
    }
    const autoPath = path.join(ROOT, 'data/dashboard/learning/automation_learning_state.json');
    ...
    if (autoState?.byKbNamespace) {
      ...
      report.push(`|---|---|---|---|---|\n`);
      for (const [ns, kb] of Object.entries(autoState.byKbNamespace)) {
        report.push(`| ${ns} | ${(kb.tunedBaseline ?? 0.6).toFixed(2)} | ${((kb.windowFpRate ?? 0) * 100).toFixed(1)}% | ${kb.windowSize ?? 0} | ${kb.reviewed ?? 0} |\n`);
      }
      report.push(`\n`);
    }
    ...
    if (fs.existsSync(trendsPath)) {
      ...
      const weekLines = lines.filter((l) => l.date >= weekAgo);
      if (weekLines.length) {
        report.push(`## Trends (last 7 days)\n\n`);
        report.push(`| KB | Days | Avg FP Rate | Avg Acceptance |\n`);
        report.push(`|---|---|---|---|\n`);
        const byNs = {};
        for (const l of weekLines) {
          (byNs[l.kbNamespace] = byNs[l.kbNamespace] || []).push(l);
        }
        for (const [ns, pts] of Object.entries(byNs)) {
          const avgFp = pts.reduce((s, p) => s + (p.fpRate ?? 0), 0) / pts.length;
          const avgAcc = pts.reduce((s, p) => s + (p.acceptanceRate ?? 0), 0) / pts.length;
          report.push(`| ${ns} | ${pts.length} | ${(avgFp * 100).toFixed(1)}% | ${(avgAcc * 100).toFixed(1)}% |\n`);
        }
        report.push(`\n`);
      }
    }
  ...
  const reportPath = path.join(ROOT, 'docs/reference/ERP_KB_Quality_Weekly_Report.md');
  fs.mkdirSync(path.dirname(reportPath), { recursive: true });
  fs.writeFileSync(reportPath, report.join(''), 'utf8');
  return reportPath;
}
