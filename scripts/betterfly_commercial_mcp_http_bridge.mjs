...
import { handleJsonRpcRequest, PROTOCOL_VERSION, SERVER_INFO } from './lib/betterfly_commercial_mcp_core.mjs';
import { getPrivacyPolicy } from './lib/betterfly_privacy.mjs';
import { getClientConfig, validateClientConfig } from './lib/betterfly_api_client.mjs';
import { verifyApiKey } from './lib/mcp_registry.mjs';

const HOST = process.env.BETTERFLY_MCP_HTTP_HOST || '127.0.0.1';
const PORT = Number(process.env.BETTERFLY_MCP_HTTP_PORT || 3500);
const MCP_PATH = process.env.BETTERFLY_MCP_HTTP_PATH || '/mcp';
const AUTH_TOKEN = process.env.BETTERFLY_MCP_HTTP_TOKEN || '';
const WRITE_AUTH_TOKEN = process.env.BETTERFLY_MCP_HTTP_WRITE_TOKEN || '';
const SSE_KEEPALIVE_MS = Number(process.env.BETTERFLY_MCP_HTTP_SSE_KEEPALIVE_MS || 15000);
const LEGACY_SSE_PATH = process.env.BETTERFLY_MCP_LEGACY_SSE_PATH || '/sse';
const MAX_BODY_BYTES = Number(process.env.BETTERFLY_MCP_HTTP_MAX_BODY_BYTES || 1048576);
const MAX_SSE_CLIENTS = Number(process.env.BETTERFLY_MCP_HTTP_MAX_SSE_CLIENTS || 50);
const REQUEST_TIMEOUT_MS = Number(process.env.BETTERFLY_MCP_HTTP_REQUEST_TIMEOUT_MS || 30000);
const HEADERS_TIMEOUT_MS = Number(process.env.BETTERFLY_MCP_HTTP_HEADERS_TIMEOUT_MS || 10000);
const REQUIRE_AUTH_ON_PUBLIC_BIND = process.env.BETTERFLY_MCP_HTTP_REQUIRE_AUTH_ON_PUBLIC_BIND !== '0';
const RATE_LIMIT_WINDOW_MS = Number(process.env.BETTERFLY_MCP_HTTP_RATE_WINDOW_MS || 60000);
const RATE_LIMIT_READ = Number(process.env.BETTERFLY_MCP_HTTP_RATE_READ || 300);
const RATE_LIMIT_WRITE = Number(process.env.BETTERFLY_MCP_HTTP_RATE_WRITE || 90);
const AUDIT_LOG_ENABLED = process.env.BETTERFLY_MCP_HTTP_AUDIT_LOG_ENABLED !== '0';
const AUDIT_LOG_PATH = process.env.BETTERFLY_MCP_HTTP_AUDIT_LOG || '/docker/openspg/data/dashboard/betterfly_mcp_http_audit.jsonl';
const AUDIT_LOG_MAX_BYTES = Number(process.env.BETTERFLY_MCP_HTTP_AUDIT_LOG_MAX_BYTES || 5 * 1024 * 1024);
const AUDIT_LOG_MAX_FILES = Math.max(1, Number(process.env.BETTERFLY_MCP_HTTP_AUDIT_LOG_MAX_FILES || 5));
...
function authContext(req) {
  if (!AUTH_TOKEN && !WRITE_AUTH_TOKEN) return { authorized: true, writeAllowed: true };
  const header = String(req.headers.authorization || '');
  const token = header.replace(/^Bearer\s+/i, '');
  const userKey = verifyApiKey(token);
  if (userKey.valid) {
    return { authorized: true, writeAllowed: !WRITE_AUTH_TOKEN, user: userKey.user.name };
  }
  if (WRITE_AUTH_TOKEN && secureTokenMatch(header, `Bearer ${WRITE_AUTH_TOKEN}`)) {
    return { authorized: true, writeAllowed: true };
  }
  if (AUTH_TOKEN && secureTokenMatch(header, `Bearer ${AUTH_TOKEN}`)) {
    return { authorized: true, writeAllowed: !WRITE_AUTH_TOKEN };
  }
  return { authorized: false, writeAllowed: false };
}

function addSseClient(res, pathName) {
...
async function handleMcpPost(req, res, auditContext = null) {
  ...
  try {
    ...
    if (error.code === 'PAYLOAD_TOO_LARGE') return payloadTooLarge(res);
...
const server = http.createServer(async (req, res) => {
  ...
  if (isMcpPath(req.url) && req.method === 'GET') return handleMcpSse(req, res);
  if (isMcpPath(req.url) && req.method === 'POST') return handleMcpPost(req, res, auditContext);
...
if (REQUIRE_AUTH_ON_PUBLIC_BIND && !isLoopbackHost(HOST) && !AUTH_TOKEN && !WRITE_AUTH_TOKEN) {
  throw new Error('Refusing to start Betterfly MCP HTTP bridge on non-loopback host without BETTERFLY_MCP_HTTP_TOKEN or BETTERFLY_MCP_HTTP_WRITE_TOKEN.');
}
...
server.listen(PORT, HOST, () => {
  process.stdout.write(
    JSON.stringify({
      ...
      privacyMode: getPrivacyPolicy().mode,
    }) + '\n',
  );
});
