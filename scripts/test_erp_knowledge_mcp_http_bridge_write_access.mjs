...
const REPO_ROOT = '/docker/openspg';
...
async function main() {
  ...
  try {
    ...
    assert.match(payload.result?.structuredContent?.jsonPath || '', /downloads\/knowledge_inbox\//);
    assert.match(payload.result?.structuredContent?.mdPath || '', /downloads\/knowledge_inbox\//);
  } finally {
    child.kill('SIGTERM');
    await wait(250);
