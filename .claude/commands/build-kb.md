...
Profile requested: $ARGUMENTS (e.g. `optima_schema_metadata`, `optima_reference`, `optima_sprint`, `optima_additional_functions`, `optima_partner_technical`, `optima_business_semantics`, `betterfly_reference`).
...
If OPENSPG_FORCE_FILES is needed, ask the user which CSV files to force-rebuild. If no profile was given in $ARGUMENTS, ask which one to use.
